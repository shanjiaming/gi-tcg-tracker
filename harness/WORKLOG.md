...
- 依据模拟器驱动重新做了完整的角色覆盖，而不是把固定夹具当成验证：新增 `coverage-generated` 串行探索器，
  修正驱动 action oneof 的 `.value` 读取，并支持指定技能/选择牌目标。114 个角色牌组、228 条 p0/p1 trace、
  945,159 条通知全部终局且双向审计通过；当前 aggregate 为 586 个目录牌、543 个 trace-observed、536 个
  有账本事件，155 个角色天赋/特技牌已全部观察到，剩余 27 个 generated-only 和 16 个 historical/runtime-missing。
  卡齐娜 `216101`、灵觉隐修的迷者生成选择 `121055/121056` 均通过额外技能/选择分支验证。
- 增加 `harness/decks/generated-only-a.json` 做父入口验证：沙中遗事、奇瑰之汤、瑟琳、乐平波琳等真实父牌
  经过 pinned simulator 触发了 `300009`、`302208`、`302226/302228`、`333022/333025/333026` 等生成牌，
  另有 12 组乐平波琳变换 trace 和 8 组父入口 trace 均审计通过。未触发的 blessing/历史牌仍按生成入口或
  当前 runtime 缺失分类记录，没有将代表牌成功扩大成全卡正确性声明。
...
- 再从 143 个新牌组中按元素和角色分散抽取 12 组，以 `cards/random` 串行跑 24 条双视角 trace、
  97,002 条通知；所有 trace 终局、ledger audit 通过、0 warnings/hidden leak。把旧矩阵、生成牌矩阵、
  定向复杂牌和本批证据合并后，当前 aggregate 为 586 个目录牌、402 个 trace-observed、398 个有账本事件；
  剩余 184 个明确分为 132 个角色天赋/特技、36 个生成实体和 16 个历史/运行时缺失，不再引用旧的 378
  数字作为当前覆盖结论。
