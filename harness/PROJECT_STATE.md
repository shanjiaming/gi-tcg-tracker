...
账本行现在还显式提供当前可见的 `pileCount`。因此不在初始牌组中的本方生成牌，只要已经公开出现在
牌库，也会进入“我牌库中的牌”；它的总牌组数仍保持 `deckCount=null`，不会被伪造为初始牌组牌，界面显示
的是当前可见数量和卡图。
...
最新合并覆盖证据（包含 114 个角色 profile、父入口牌组和技能/选择分支 trace）为 586 个目录牌中 543 个
`trace-observed`、536 个产生账本事件；43 个未观察牌分为 27 个 `generated-only` 和 16 个
`historical/runtime-missing`，155 个角色天赋/特技牌已经全部有公开观察证据。父入口验证实际触发了
`300009`、`302208`、`302226/302228`、`333022/333025/333026` 等生成牌；仍不能把未触发的 blessing
条件分支或当前 runtime 缺失牌宣称为全牌正确。生成器现按角色源码和上游 `cards/` 源码文件建立 149 个
profile；后者只登记生成入口，不把不可直接构筑的实体伪装成初始牌组。
...
## 2026-07-16 simulator-driven coverage snapshot

本轮重新使用 pinned upstream simulator 逐局驱动，而不是静态 fixture：新增目标卡顺序策略、元素调和目标手牌
优先级，以及三组可重复的生成链 harness。`npm run coverage-blessing` 以 8 个元素祝佑场景运行 16 局、
双方 perspective trace 全部 audit-clean；`npm run coverage-remaining` 运行幻戏倒计时弃置链、乐平波琳随机计划
链和自体自身之塔冒险链，3 组均通过双方视角终局审计。证据脚本为
`scripts/run-generated-blessing-exploration.ts` 和 `scripts/run-generated-remaining-exploration.ts`。

最新 `records/coverage/card-coverage-aggregate-20260820.json`：586 张目录牌、570 张 trace-observed、565 张
产生账本事件；996 份 trace、4,179,067 条通知、missingFiles=0。运行时仍存在的 570 张牌全部已在 trace 中
出现；剩余 16 张是 runtime 缺失的 historical/runtime-missing ID，不把它们伪造为当前可触发牌。这里仍是路径和
账本证据，不是对每张卡的全部数值/条件效果作形式化等价证明。

本轮固定复现的高风险路径包括：331004 的超导祝佑两分支、332032→332033→332034→332035 的真实弃置链、
322033→302230→302224 的随机计划变换，以及 321033 冒险进度 5/12 生成 301038/301039。`npm test` 为
50/50，`npm run verify` 和 `npm run stress` 均通过；stress 为 12 条 trace、81,464 条通知，默认 verify 为
12 条 trace、34,896 条通知。生成牌组 `harness/decks/generated-adventure.json` 只服务于可重复模拟器探索。
...
## 2026-07-17 realtime simulator bridge verification

新增 `scripts/run-live-simulator.ts` 和 `npm run live-simulator`。它启动一个临时本地 tracker，
由 `generate-simulator-trace.ts` 驱动一局真实 pinned `Game`，在每个模拟器 RPC 边界刷新通知队列，
并将双方的 deck/session 与公开 notification 实时送入正式 `/api/session`、`/api/ingest` 链路。
这不是静态 fixture，也不包含动作控制。

seed `20260730`、`standard-a.json`/`standard-b.json`、`cards`/`skills` 双策略运行结果：p0/p1
都到 phase 5 且 warnings=0；实时序列为 2810/2809，原始 trace 序列为 3022/3021。实时最终快照
与对应 trace 离线回放的 `phase/roundNumber/currentTurn/winner/sides/cards/warnings` 投影两侧均
`sameProjection=true`。两条 trace 的直接 audit 也均为 terminal、errors=0、masked-state leaks=0、
masked-snapshot leaks=0、warnings=0。
