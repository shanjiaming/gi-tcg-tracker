...
最新合并覆盖证据（包含 474 条 trace 文件、1,726,492 条通知）为 586 个目录牌中 402 个
`trace-observed`、398 个产生账本事件；184 个未观察牌不是失败吞掉，而是 132 个角色天赋/特技、36 个
生成实体和 16 个历史/运行时缺失。直接可构筑牌已在这份 aggregate 中全部有公开观察证据，但条件分支仍需
继续用定向真实对局补齐，不能将此 aggregate 宣称为全牌/全条件正确。
