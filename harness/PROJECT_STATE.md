...
最新合并覆盖证据（包含既有 trace 与本轮 12 个定向 profile 的新增 trace）为 586 个目录牌中 411 个
`trace-observed`、407 个产生账本事件；175 个未观察牌不是失败吞掉，而是 123 个角色天赋/特技、36 个
生成实体和 16 个历史/运行时缺失。直接可构筑牌已在这份 aggregate 中全部有公开观察证据，但条件分支仍需
继续用定向真实对局补齐，不能将此 aggregate 宣称为全牌/全条件正确。生成器现按角色源码和上游 `cards/`
源码文件建立 149 个 profile；后者只登记生成入口，不把不可直接构筑的实体伪装成初始牌组。
