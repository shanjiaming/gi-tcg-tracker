...
test("page collector renders a scrollable read-only overlay and stays action-free", async () => {
  ...
  assert.match(source, /toggle\.setAttribute\("aria-label", "收起记牌器"\)/);
  assert.match(source, /overlayCollapsed = !overlayCollapsed/);
  assert.match(source, /overlayBody\.style\.display = overlayCollapsed \? "none" : "block"/);
  assert.match(source, /toggle\.textContent = overlayCollapsed \? "展开" : "收起"/);
  assert.match(source, /toggle\.setAttribute\("aria-label", overlayCollapsed \? "展开记牌器" : "收起记牌器"\)/);
  assert.match(source, /overlayLayoutStorageKey/);
  assert.match(source, /localStorage\.getItem\(overlayLayoutStorageKey\)/);
  assert.match(source, /localStorage\.setItem\(overlayLayoutStorageKey/);
  assert.match(source, /拖动记牌器窗口/);
  assert.match(source, /beginGesture/);
  assert.match(source, /pointerdown/);
  assert.match(source, /调整记牌器大小/);
  assert.match(source, /cursor:nwse-resize/);
  assert.match(source, /grid-template-columns:repeat\(auto-fill,minmax\(82px,1fr\)\)/);
  assert.match(source, /aspect-ratio:5 \/ 7/);
  assert.match(source, /object-fit:contain/);
  assert.match(source, /pointer-events:none/);
  assert.match(source, /height:720px/);
